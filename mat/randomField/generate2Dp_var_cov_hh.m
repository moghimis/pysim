function A=gegenerate2Dp_var_cov_hh(nx,ny,dx,dy,Lx,Ly,Lz,N,hp)
%
...
if (var_covhh)
    ...
    %infile='prior.nc';
    %hp = nc_varget(infile,'h');
    hp(hp==-10)=0.1;
    alpha =zeros(size(hp));
    hmin=1;
    alpha (hp > hmin) = 1.0;
    ind = find(hp < hmin);
    alpha (ind) = sin(pi * hp(ind) /(hmin *2));
end
...
%%
if (var_covhh)
    alphac = ones(n1,n2)* min(min(alpha)) ;
    alphac(n1/2-nx/2+1:n1/2+nx/2  , n2/2-ny/2+1:n2/2+ny/2) = alpha;
    for i=1:n1/2-nx/2 
      alphac(i,:) = alphac(n1/2-nx/2+1+1,:);
    end

    for i=n1/2+nx/2+1:n1
      alphac(i,:) = alphac(n1/2+nx/2,:);
    end

    for j=1:n2/2-ny/2 
      alphac(:,j) = alphac(:,n2/2-ny/2+1+1);
    end

    for j=n2/2+ny/2+1:n2
      alphac(:,j) = alphac(:,n2/2+ny/2);
    end
end%%
% define constantsalpha(1,:)
pi2=2.0*pi;
...
parfor nn=1:nreal
  ...
  if (var_covhh)
      qhat = qhat .*alphac;    %% add by saeed for variable cov_hh
  end
